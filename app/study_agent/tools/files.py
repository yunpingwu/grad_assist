...
if __name__ == "__main__":
    ...
    async def _run() -> None:
        with tempfile.TemporaryDirectory() as tmp:
            import app.study_agent.tools.files as files_mod

            files_mod.MATERIAL_ROOT = Path(tmp)  # 桩根目录，隔离真实落盘
            tb, tk = "测试教材", "t1"

            # 1) 越界防线
            for bad in ("../逃逸.md", "子/../../逃逸.md"):
                try:
                    await files_mod.write_file.coroutine(filename=bad, content="x", textbook_name=tb, task_id=tk)
                    raise AssertionError(f"应拒绝越界路径: {bad}")
                except ValueError:
                    pass
            try:
                await files_mod.write_file.coroutine(filename="/abs.md", content="x", textbook_name=tb, task_id=tk)
            ...
            try:
                await files_mod.edit_file.coroutine(filename="第1章/总结.md", old_string="正文A", new_string="乙", textbook_name=tb, task_id=tk)
                raise AssertionError("不唯一匹配应报错")
            except ValueError:
                pass
            # replace_all 生效
            edited = await files_mod.edit_file.coroutine(
            ...
            # 3) 无 .tmp 残留
            leftovers = list((Path(tmp) / tb / tk).rglob("*.tmp"))
