...
async def mineru_upload_and_poll(toc_files: list[Path], toc_dir: Path) -> list[str]:
    """利用 mineru 解析切割的目录 pdf 文件。
...
async def split_contents(state: TextBookState, *, writer: StreamWriter) -> dict:
    ...
    # 获取解析结果
    full_zip_urls = await mineru_upload_and_poll(toc_files, toc_dir)
    # 下载并解压解析结果（用教材名命名目录）
    toc_names = [f.stem.replace("_toc", "") for f in toc_files]
    output_dir = toc_dir.parent / "mineru_toc"
    extracted_dirs = await mineru_download_and_extract(full_zip_urls, output_dir, names=toc_names)
    ...
    writer(
        {
            "type": "message",
            "status": "running",
            "message": f"目录解析完成，共 {len(extracted_dirs)} 本教材",
            "progress": 0.4,
        }
    )
    return state
